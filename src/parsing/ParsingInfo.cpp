...
ParsingInfo::ParsingInfo(): _movement(ParsingMovement::TO_DEFAULT),
                            _line_id(0),
                            _position_in_line(0),
                            _words(),
                            _position(nullptr),
...
void ParsingInfo::set_movement(ParsingMovement movement) { this->_movement = movement;}
void ParsingInfo::set_line_id(int const& line_id) { this->_line_id = line_id;}
void ParsingInfo::shift_position_to(char const* position) {
    auto prev = this->_position;
    this->_position = position;
    this->_position_in_line += position - prev;
}

void ParsingInfo::push_word(Word* word) {this->_words.push_back(std::shared_ptr<Word>(word));}

std::vector<std::shared_ptr<Word>> ParsingInfo::words() const {return this->_words;}

ParsingMovement ParsingInfo::movement() const {return this->_movement;}
const char* ParsingInfo::position() const {return this->_position;}
ParsingState* ParsingInfo::get_state() {
...
void ParsingInfo::parse(std::string& str) {
    this->_line_id = 0;
